...
angular.module('chapelProject')
  .config(function($stateProvider) {
    ...
    $stateProvider
      .state('login', {
        ...
        controllerAs: 'login',
        title:'Login to User Management'
      })
      .state('logout', {
        url: '/logout?referrer',
        referrer: '/',
      ...
      .state('signup', {
        ...
        controllerAs: 'signup',
        title:'Signup for chapelProject'
      })
      .state('forgot', {
        url: '/forgot?email',
  ...
  .run(function($rootScope, Auth) {
    ...
    $rootScope.$on('$stateChangeSuccess', function (evt, toState) {
      if(toState.title){
        window.document.title = toState.title + ' - chapelProject';
      }else if(toState.name != 'crud-table'){
        var input = toState.name;
        input = input.replace(/([A-Z])/g, ' $1');
        input = input[0].toUpperCase() + input.slice(1);
        window.document.title = input + ' - chapelProject';
      }
    });
  });
